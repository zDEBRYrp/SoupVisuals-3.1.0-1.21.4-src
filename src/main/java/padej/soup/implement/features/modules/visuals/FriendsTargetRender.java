...
public class FriendsTargetRender extends Module implements ITargetRenderModule {
   ...
   private final ValueSetting ghostsRadiusModifier = new ValueSetting(
      ...
      .setValue(1.6F)
      .range(0.5F, 2.0F);
   private final ValueSetting ghostsEndSize = new ValueSetting(
         "setting.friendstargetrender.ghostsstartsize.name", "setting.friendstargetrender.ghostsstartsize.desc"
      )
      .setValue(0.1F)
      .range(0.1F, 1.0F);
   private final ValueSetting ghostsStartSize = new ValueSetting(
         "setting.friendstargetrender.ghostsendsize.name", "setting.friendstargetrender.ghostsendsize.desc"
      )
      .setValue(0.7F)
      .range(0.1F, 1.0F);
   ...
   public FriendsTargetRender() {
      ...
      this.setup(colorGroup, this.targetEspType, legacyGroup, ghostsGroup, crystalsGroup, behaviorGroup);
   }
   ...
   public void onWorldRender(WorldRenderEvent e) {
      if (ServerLimitCfg.showFriendsTargetRender()) {
         ...
         Map<LivingEntity, FriendsTargetRender.TargetRenderData> uniqueTargets = new HashMap<>();
         this.friendTargets.forEach((uuid, data) -> {
            LivingEntity target = data.targetEntity != null ? data.targetEntity : data.lastTargetEntity;
            if (target == null || this.shouldShowTarget(target)) {
               boolean shouldRender = data.targetEntity != null && this.canFriendSeeTarget(data.friend, data.targetEntity);
               if (data.targetEntity == null && data.lastTargetEntity != null) {
                  shouldRender = currentTime - data.lastTargetTime < followTimeMs && this.canFriendSeeTarget(data.friend, data.lastTargetEntity);
                  if (data.lastTargetEntity.isDead() || !data.lastTargetEntity.isAlive()) {
                     shouldRender = false;
                  }
               }

               data.renderAnimation.setDirection(shouldRender ? Direction.FORWARDS : Direction.BACKWARDS);
               float animationDelta = data.renderAnimation.getOutput().floatValue();
               if (data.targetEntity != null) {
                  data.lastTargetEntity = data.targetEntity;
               }

               if (data.lastTargetEntity != null && !data.renderAnimation.isFinished(Direction.BACKWARDS)) {
                  ...
                  FriendsTargetRender.TargetRenderData existingData = uniqueTargets.get(data.lastTargetEntity);
                  if (existingData == null || animationDelta > existingData.animationDelta) {
                     uniqueTargets.put(data.lastTargetEntity, new FriendsTargetRender.TargetRenderData(data.lastTargetEntity, animationDelta));
                  }
               }
            }
         });
         ...
         for (AbstractClientPlayerEntity player : mc.world.getPlayers()) {
            if (FriendUtils.isFriend(player) && player != mc.player) {
               if (!this.isWithinMaxDistance(player)) {
                  ...
                  if (previousTarget != null && data.targetEntity == null) {
                     data.lastTargetTime = System.currentTimeMillis();
                  }
               }
            }
   ...
   private boolean canFriendSeeTarget(AbstractClientPlayerEntity friend, LivingEntity target) {
      ...
      if (this.isWithinMaxDistance(friend) && this.isWithinMaxDistance(target)) {
   ...
   public boolean isFriendTargeting(LivingEntity entity) {
      ...
      for (FriendsTargetRender.FriendTargetData data : this.friendTargets.values()) {
         if (data.targetEntity == entity || data.lastTargetEntity == entity) {
            return true;
         }
      }

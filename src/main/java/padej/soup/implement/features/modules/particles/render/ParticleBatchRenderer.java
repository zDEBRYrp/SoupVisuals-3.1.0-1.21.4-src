...
public class ParticleBatchRenderer implements QuickImports {
   ...
   private static final Queue<ParticleBatchRenderer.TexturedParticleData> TEXTURED_PARTICLES;
   private static final Queue<ParticleBatchRenderer.CubeParticleData> CUBE_PARTICLES;
   private static final Queue<ParticleBatchRenderer.PyramidParticleData> PYRAMID_PARTICLES;
   private static final Queue<ParticleBatchRenderer.TextParticleData> TEXT_PARTICLES;
   private static final Queue<ParticleBatchRenderer.BloomParticleData> BLOOM_PARTICLES;
   private static final Queue<ParticleBatchRenderer.FireflyTrailData> FIREFLY_TRAILS;
   private static final Queue<ParticleBatchRenderer.BubbleParticleData> BUBBLE_PARTICLES;
   private static final Queue<ParticleBatchRenderer.GhostParticleData> GHOST_PARTICLES;
   ...
   private static final Map<String, List<ParticleBatchRenderer.GhostParticleData>> GHOST_GROUPS;
   private static final float SQRT_3;
   private static final float SQRT_6;
   private static final Queue<ParticleBatchRenderer.AmbientTexturedParticleData> AMBIENT_TEXTURED_PARTICLES;
   ...
   private static void renderGhostBatch() {
      ...
      while (!GHOST_PARTICLES.isEmpty()) {
         ...
         String key = data.texture.toString() + "|" + data.blendMode;
         GHOST_GROUPS.computeIfAbsent(key, k -> new ArrayList<>()).add(data);
      }

      GHOST_GROUPS.forEach((keyx, ghosts) -> {
         if (!ghosts.isEmpty()) {
   ...
   static {
      ...
      TEXTURED_PARTICLES = new ConcurrentLinkedQueue<>();
      CUBE_PARTICLES = new ConcurrentLinkedQueue<>();
      PYRAMID_PARTICLES = new ConcurrentLinkedQueue<>();
      TEXT_PARTICLES = new ConcurrentLinkedQueue<>();
      BLOOM_PARTICLES = new ConcurrentLinkedQueue<>();
      FIREFLY_TRAILS = new ConcurrentLinkedQueue<>();
      BUBBLE_PARTICLES = new ConcurrentLinkedQueue<>();
      GHOST_PARTICLES = new ConcurrentLinkedQueue<>();
      ...
      AMBIENT_TEXTURED_PARTICLES = new ConcurrentLinkedQueue<>();
   }

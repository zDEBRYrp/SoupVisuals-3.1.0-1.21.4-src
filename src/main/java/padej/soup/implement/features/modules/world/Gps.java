   ...
   private void renderLocationIcon(MatrixStack matrix) {
      ...
      if (screenPos.z > 0.0) {
         ...
         if (screenPos.x >= -margin && screenPos.x <= screenWidth + margin && screenPos.y >= -margin && screenPos.y <= screenHeight + margin) {
            ...
            RenderSystem.disableDepthTest();
            RenderSystem.blendFunc(SrcFactor.SRC_ALPHA, DstFactor.ONE_MINUS_CONSTANT_ALPHA);
            RenderSystem.setShaderTexture(0, Identifier.of("textures/loc.png"));
            RenderSystem.setShader(ShaderProgramKeys.POSITION_TEX_COLOR);
            BufferBuilder buffer = Tessellator.getInstance().begin(DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
            int color = ColorUtil.getClientColor();
            matrix.push();
            ...
            matrix.translate(-x, -y, 0.0F);
            Matrix4f matrix4f = matrix.peek().getPositionMatrix();
            buffer.vertex(matrix4f, x - size / 2.0F, y + size, 0.0F).texture(0.0F, 1.0F).color(ColorUtil.multAlpha(ColorUtil.multDark(color, 0.4F), 0.5F));
            buffer.vertex(matrix4f, x + size / 2.0F, y + size, 0.0F).texture(1.0F, 1.0F).color(ColorUtil.multAlpha(ColorUtil.multDark(color, 0.4F), 0.5F));
            buffer.vertex(matrix4f, x + size / 2.0F, y, 0.0F).texture(1.0F, 0.0F).color(color);
            buffer.vertex(matrix4f, x - size / 2.0F, y, 0.0F).texture(0.0F, 0.0F).color(color);
            BufferRenderer.drawWithGlobalProgram(buffer.end());
            RenderSystem.enableDepthTest();
            RenderSystem.enableCull();
            RenderSystem.defaultBlendFunc();

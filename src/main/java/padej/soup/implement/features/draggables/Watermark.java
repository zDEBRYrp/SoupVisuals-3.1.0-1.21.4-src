...
public class Watermark extends AbstractDraggable {
   ...
   private void drawMonoStyle(MatrixStack matrix, FontRenderer font, List<WatermarkComponent> enabledComponents) {
      ...
      for (int lineIndex = 0; lineIndex < lines.size(); lineIndex++) {
         ...
         for (int i = 0; i < line.size(); i++) {
            ...
            boolean isIconInList = i == 0 && hasIconInLine;
            if (!isIconInList && i < line.size() - 1) {
   ...
   private List<WatermarkComponent> getEnabledComponents() {
      return this.components.stream().filter(component -> this.isComponentEnabled(component.getName())).collect(Collectors.toList());
   }

...
public class Potions extends AbstractDraggable {
   ...
   public void packet(PacketEvent e) {
      switch (e.getPacket()) {
         ...
         case RemoveEntityStatusEffectS2CPacket effect:
            String effectTypeId = effect.effect().getIdAsString();
         ...
         case PlayerRespawnS2CPacket ignored:
            this.list.clear();
            break;
         case GameJoinS2CPacket ignored:
            this.list.clear();
            break;
         default:
      }
   ...
   public void drawDraggable(DrawContext context) {
      ...
      if (showHeader) {
         ...
         blur.render(
            ShapeProperties.create(matrix, this.getX(), this.getY() + headerHeight, this.getWidth(), this.getHeight() - headerHeight)
      ...
      for (Potions.Potion potion : this.list) {
         ...
         MathUtil.scale(matrix, centerX, centerY, 1.0F, animation, () -> {
            Render2DUtil.drawSprite(matrix, mc.getStatusEffectSpriteManager().getSprite(effect.getEffectType()), this.getX() + 5, (int)centerY - 2, 8.0F, 8);
            ...
            int nameColor;
            if (potionsModule.getColoredEffects().isValue()) {
               boolean isBeneficial = effect.getEffectType().value().isBeneficial();
               nameColor = isBeneficial ? -11141291 : -43691;
            } else {
               nameColor = ColorUtil.getText();
            }

            int durationColor;
            if (effect.getDuration() != -1 && effect.getDuration() <= 120) {

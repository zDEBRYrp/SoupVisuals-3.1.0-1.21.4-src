...
public class ConfigManagerComponent extends AbstractComponent {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      rectangle.render(
         ShapeProperties.create(matrix, this.x, this.y, this.width, this.height)
            ...
            .color(ColorUtil.getGuiRectColor(0.0F))
            .build()
      );
            ...
            if (!(entryY + 14.0F < listStartY) && !(entryY > listStartY + listHeight)) {
               ...
               MathUtil.scale(
                  ...
                  () -> {
                     ...
                     image.setTexture("textures/edit.png")
                        .render(ShapeProperties.create(matrix, editX, iconY, iconSize, iconSize).color(editIconColor).build());
                     ...
                     image.setTexture("textures/trash.png")
                        .render(ShapeProperties.create(matrix, deleteX, iconY, iconSize, iconSize).color(deleteIconColor).build());
                  }
               );
            }
         }
   ...
   private void saveCurrentConfig(String configName) {
      try {
         ...
         LoggerUtil.error("Failed to save current config: " + configName, e);
      }
   }
   ...
   private void loadConfig(String configName) {
      try {
         ...
         LoggerUtil.error("Failed to load config: " + configName, e);
      }
   }
   ...
   private void removeConfig(String configName) {
      if (ConfigUtils.canDeleteConfig()) {
         ...
         try {
            ...
            LoggerUtil.error("Failed to remove config: " + configName, e);
         }
      }
   }
   ...
   public void saveActiveConfigIfExists() {
      try {
         ...
         LoggerUtil.error("Failed to save active config", e);
      }
   }
      ...
      try {
         ...
         try {
            ...
            try {
               ...
               LoggerUtil.error("Failed to copy config file during rename", e2);
               if (newFile.exists()) {
                  newFile.delete();
               }
            }
         ...
         if (renamed) {
            ...
            if (!capitalizedNewName.isEmpty()) {
               capitalizedNewName = capitalizedNewName.substring(0, 1).toUpperCase() + capitalizedNewName.substring(1);
            }
         ...
         LoggerUtil.error("Failed to rename config from " + oldName + " to " + newName, e);
      }
   }

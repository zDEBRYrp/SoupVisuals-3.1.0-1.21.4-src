...
public class ValueComponent extends AbstractSettingComponent {
   ...
   private float getDifference(int mouseX, MatrixStack matrix) {
      ...
      this.animation = MathUtil.interpolate(this.animation, percentValue);
   ...
   private void changeValue(float difference) {
      BigDecimal bd = BigDecimal.valueOf(difference / 45.0F * (this.setting.getMax() - this.setting.getMin()) + this.setting.getMin())

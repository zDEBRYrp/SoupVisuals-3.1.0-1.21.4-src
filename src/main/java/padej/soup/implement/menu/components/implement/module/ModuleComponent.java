...
public class ModuleComponent extends AbstractComponent {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      rectangle.render(
         ShapeProperties.create(context.getMatrices(), this.x, this.y, this.width, this.height = this.getComponentHeight())
            ...
            .color(ColorUtil.getRectDarker(0.0F))
            .build()
      );
      if (this.canCollapse()) {
      ...
      if (this.module.isShowEnable() && this.module.isCanBind()) {
         ...
         float bindBadgeLeft = this.x + this.width - Fonts.getSize(12, Fonts.Type.INTER_BOLD).getStringWidth(bindDisplayName) - 15.0F;
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      ...
      if (this.collapseAnimation.getOutput().floatValue() > 0.5F) {
         boolean isAnyComponentHovered = this.components.stream().anyMatch(abstractComponentx -> abstractComponentx.isHover(mouseX, mouseY));
   ...
   private void drawBind(DrawContext context, float renderX, float renderY) {
      ...
      float stringWidth = Fonts.getSize(12, Fonts.Type.INTER_BOLD).getStringWidth(name);
      rectangle.render(
         ShapeProperties.create(context.getMatrices(), renderX + this.width - stringWidth - 15.0F, renderY + 4.5F, stringWidth + 6.0F, 9.0)
            .round(2.0F)
            .thickness(2.0F)
      ...
      Fonts.getSize(12, Fonts.Type.INTER_BOLD)
         .drawString(context.getMatrices(), name, renderX + this.width - 12.0F - stringWidth, renderY + 8.0F, bindingColor);
   }
   ...
   private void copyModuleConfig() {
      ...
      if (currentTime - this.lastCopyTime >= 150L) {
         ...
         try {
            ...
            LoggerUtil.error("Failed to copy module config", e);
         }
      }
   }
         ...
         try {
            ...
            for (Setting setting : this.module.settings()) {
               try {
                  ...
                  LoggerUtil.error("Failed to load setting: " + setting.getNameKey(), e);
               }
            }
            ...
            LoggerUtil.error("Failed to paste module config", e);
         }
      }
   }

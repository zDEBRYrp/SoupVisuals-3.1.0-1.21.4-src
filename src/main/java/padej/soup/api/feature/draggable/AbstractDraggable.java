...
public abstract class AbstractDraggable implements Draggable, QuickImports, QuickLogger {
         ...
         for (AbstractDraggable drag : Main.getInstance().getDraggableRepository().draggable()) {
            if (this.canDraw(drag) && drag.canDrag && drag != this) {
               ...
               if (Math.abs(y3 - mouseDragY) <= radius) {
                  this.drawRect(0.0F, y3 - 1.5F, windowWidth, 1.0F);
                  clampedY = y3;
               }
            }
         }
   ...
   private void saveDraggablePositions() {
      try {
         ...
         LoggerUtil.error("Failed to save draggable positions: " + e.getMessage());
      }
   }
   ...
   public boolean canDraw(AbstractDraggable draggable) {
      Module module = Main.getInstance().getModuleProvider().get(draggable.getName());
   ...
   protected float getInteractionScale() {
      Module module = Main.getInstance().getModuleProvider().get(this.getName());
      if (module == null) {
         return 1.0F;
      }

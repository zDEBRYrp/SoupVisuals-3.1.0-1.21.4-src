...
public final class TargetRenderer implements QuickImports {
   ...
   public static void drawCircle(MatrixStack matrix, LivingEntity target, float animation, float red, ITargetRenderModule module) {
      ...
      for (int size = 90; i <= size; i++) {
         float width = target.getWidth() * 0.9F;
         ...
         Render3DUtil.vertexLine(
            matrix,
            buffer,
            targetPos.add(cosSin.x, cosSin.y + yAnim, cosSin.z),
            targetPos.add(cosSin.x, cosSin.y + yAnim2, cosSin.z),
            ColorUtil.multAlpha(color, 0.6F * animation),
            ColorUtil.multAlpha(color, 0.0F)
         );
         Render3DUtil.drawLine(
            targetPos.add(cosSin.x, cosSin.y + yAnim, cosSin.z),
            targetPos.add(nextCosSin.x, nextCosSin.y + yAnim, nextCosSin.z),
            ColorUtil.multAlpha(color, animation),
            3.0F,
            true
         );
      }
   ...
   public static void drawGhosts(
      ...
      Camera camera = mc.getEntityRenderDispatcher().camera;
      Vec3d vec = MathUtil.interpolate(target).subtract(camera.getPos());
      double interpolate = MathUtil.interpolate(mc.player.age - 1, mc.player.age);
         ...
         for (int i = 0; i < baseGhostLength; i++) {
            for (int sub = 0; sub < subdivisionsInt; sub++) {
               ...
               if (isSpiral) {
                  ...
                  double orbitY = tempY;
                  double orbitZ = tempZ;
                  matrices.translate(vec.x + orbitX, vec.y + halfHeight + orbitY, vec.z + orbitZ);
               }

               matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(-camera.getYaw()));
               matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(camera.getPitch()));
               float trailGradient = 1.0F - currentStep / baseGhostLength;

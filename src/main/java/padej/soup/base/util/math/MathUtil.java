...
public final class MathUtil implements QuickImports {
   ...
   public static Vec3d closestPointToEntity(Entity entity) {
      ...
      Vec3d prevClosest = new Vec3d(
         Math.min(Math.max(eye.x, prevBox.minX), prevBox.maxX),
         Math.min(Math.max(eye.y, prevBox.minY), prevBox.maxY),
         Math.min(Math.max(eye.z, prevBox.minZ), prevBox.maxZ)
      );
      Vec3d currentClosest = new Vec3d(
         Math.min(Math.max(eye.x, currentBox.minX), currentBox.maxX),
         Math.min(Math.max(eye.y, currentBox.minY), currentBox.maxY),
         Math.min(Math.max(eye.z, currentBox.minZ), currentBox.maxZ)
      );
      return interpolate(prevClosest, currentClosest);
   }
